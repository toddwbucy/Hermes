...
import (
	...
	"github.com/toddwbucy/hermes/internal/palette"
)
...
func (p *Plugin) doCreateWorktree(name, baseBranch, taskID, taskTitle string, agentType AgentType) (*Worktree, error) {
	...
	// If task is linked, create .hermes-task file
	if taskID != "" {
		taskPath := filepath.Join(wtPath, sidecarTaskFile)
		if err := os.WriteFile(taskPath, []byte(taskID+"\n"), 0644); err != nil {
			p.ctx.Logger.Warn("failed to write .hermes-task", "path", taskPath, "error", err)
		}
	}
...
const sidecarTaskFile = ".hermes-task"
const sidecarAgentFile = ".hermes-agent"
const sidecarPRFile = ".hermes-pr"
const sidecarBaseFile = ".hermes-base"
...
// loadTaskLink reads the linked task ID from the .hermes-task file.
...
// linkTask returns a command to link a task to a worktree.
	...
	return func() tea.Msg {
		...
		if err := os.WriteFile(taskPath, []byte(taskID+"\n"), 0644); err != nil {
			return TaskLinkedMsg{
				WorkspaceName: wt.Name,
				Err:           fmt.Errorf("write .hermes-task: %w", err),
			}
		}
...
func (p *Plugin) unlinkTask(wt *Worktree) tea.Cmd {
	return func() tea.Msg {
		...
		if err := os.Remove(taskPath); err != nil && !os.IsNotExist(err) {
			return TaskLinkedMsg{
				WorkspaceName: wt.Name,
				Err:           fmt.Errorf("remove .hermes-task: %w", err),
			}
		}
...
// loadOpenTasks returns an empty task list (td integration removed).
// TODO: Replace with Persephone task loading via ArangoDB.
func (p *Plugin) loadOpenTasks() tea.Cmd {
	return func() tea.Msg {
		return TaskSearchResultsMsg{Tasks: []Task{}}
	}
}
...
// loadTaskDetails returns empty details (td integration removed).
// TODO: Replace with Persephone task detail loading via ArangoDB.
func (p *Plugin) loadTaskDetails(taskID string) tea.Cmd {
	return func() tea.Msg {
		return TaskDetailsLoadedMsg{TaskID: taskID, Err: fmt.Errorf("task details not available (td removed)")}
	}
}
