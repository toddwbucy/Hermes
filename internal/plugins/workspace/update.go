...
import (
	...
	app "github.com/toddwbucy/hermes/internal/app"
	"github.com/toddwbucy/hermes/internal/plugin"
	"github.com/toddwbucy/hermes/internal/plugins/gitstatus"
)
...
func (p *Plugin) Update(msg tea.Msg) (plugin.Plugin, tea.Cmd) {
	...
	case RefreshDoneMsg:
		...
		if msg.Err == nil {
			...
			for _, wt := range p.worktrees {
				...
				// Load linked task ID from .hermes-task file
				wt.TaskID = loadTaskLink(wt.Path)
				// Load chosen agent type from .hermes-agent file
				wt.ChosenAgentType = loadAgentType(wt.Path)
				// Load PR URL from .hermes-pr file
				wt.PRURL = loadPRURL(wt.Path)
				// Load base branch from .hermes-base file
	...
	case reconnectedAgentsMsg:
		...
		// (worktrees with .hermes-agent file but no tmux session)
