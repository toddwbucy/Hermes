...
func TestWriteAgentLauncher(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			baseCmd:   "claude",
			prompt:    "Task: fix bug",
			wantCmd:   "bash '" + tmpDir + "/.hermes-start.sh'",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			launcherInfo, err := os.Stat(tmpDir + "/.hermes-start.sh")
			if err != nil {
			...
			scriptContent, err := os.ReadFile(tmpDir + "/.hermes-start.sh")
			if err != nil {
			...
			_ = os.Remove(tmpDir + "/.hermes-start.sh")
		})
	}
}
