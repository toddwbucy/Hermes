...
install-dev:
	...
	@echo "Installing hermes with Version=$(VERSION)"
	go install -ldflags "-X main.Version=$(VERSION)" ./cmd/hermes
...
fmt-check-all:
	@unformatted="$$(find . -name '*.go' -not -path './vendor/*' -not -path './website/*' -not -path './sidecar/*' -print0 | xargs -0 gofmt -l)"; \
	if [ -n "$$unformatted" ]; then \
...
build-all:
	GOOS=darwin GOARCH=amd64 go build -o bin/hermes-darwin-amd64 ./cmd/hermes
	...
	GOOS=linux GOARCH=arm64 go build -o bin/hermes-linux-arm64 ./cmd/hermes
