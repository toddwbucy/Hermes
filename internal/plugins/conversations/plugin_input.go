...
func (p *Plugin) updateSessions(msg tea.KeyMsg) (plugin.Plugin, tea.Cmd) {
	...
	case "W":
		// Quick-toggle workspace CWD filter (td-0ea560)
		return p, p.toggleWorkspaceFilter()
...
// toggleWorkspaceFilter toggles between workspace-scoped and all-workspace session views (td-0ea560).
func (p *Plugin) toggleWorkspaceFilter() tea.Cmd {
	if p.filters.WorkspaceCWD != "" {
		// Currently filtered -> show all workspaces
		p.filters.WorkspaceCWD = ""
		p.filterActive = p.filters.IsActive()
		p.cursor = 0
		p.scrollOff = 0
		p.hitRegionsDirty = true
		return appmsg.ShowToast("Showing all workspaces", 2*time.Second)
	}

	// Currently showing all -> filter to current workspace
	p.filters.WorkspaceCWD = p.ctx.WorkDir
	p.filterActive = p.filters.IsActive()
	p.cursor = 0
	p.scrollOff = 0
	p.hitRegionsDirty = true
	return appmsg.ShowToast("Showing workspace sessions only", 2*time.Second)
}
