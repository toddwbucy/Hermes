...
func TestInitDoesNotApplyCategoryFilter(t *testing.T) {
	...
	// filterActive should be true because workspace filter is on by default (td-0ea560)
	if !p.filterActive {
		t.Error("filterActive should be true after Init (workspace filter is default-on)")
	}

	// WorkspaceCWD should match the context WorkDir
	if p.filters.WorkspaceCWD != "/test/project" {
		t.Errorf("WorkspaceCWD = %q, want /test/project", p.filters.WorkspaceCWD)
	}
}
