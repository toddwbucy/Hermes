...
func (p *Plugin) renderWelcomeGuide(width, height int) string {
	...
	if !isTmuxInstalled() {
		...
		lines = append(lines, "")
		lines = append(lines, dimText("After installing, restart hermes to use this feature."))
...
func (p *Plugin) renderShellPrimer(width, height int) string {
	...
	lines = append(lines, dimText(fmt.Sprintf("  %s d      Detach (return to hermes)", prefix)))
	lines = append(lines, "")
