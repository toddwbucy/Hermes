...
// If sessionID is empty, falls back to "hermes".
func NewStore(dbPath, sessionID string) (*Store, error) {
	...
	// Resolve session ID: param > "hermes" default
	if sessionID == "" {
		sessionID = "hermes"
	}
...
// Notes are stored in .hermes/notes.db in the project root.
func DefaultDBPath(workDir string) string {
	return filepath.Join(workDir, ".hermes", "notes.db")
}
