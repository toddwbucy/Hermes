...
import (
	...
	"github.com/toddwbucy/hermes/internal/features"
)
...
func (p *Plugin) writeAgentLauncher(worktreePath string, agentType AgentType, baseCmd, prompt string) (string, error) {
	launcherFile := filepath.Join(worktreePath, ".hermes-start.sh")
...
// getTaskContext returns task context for agent startup.
// TODO: Replace with Persephone task context loading via ArangoDB.
func (p *Plugin) getTaskContext(taskID string) string {
	// td integration removed - return empty to use fallback (task title from modal)
	return ""
}
...
func (p *Plugin) detectOrphanedWorktrees() {
	for _, wt := range p.worktrees {
		...
		if wt.IsMain {
			wt.IsOrphaned = false
			// Clean up any stale .hermes-agent file from main worktree
