...
// Hermes files that should be in .gitignore
...
func (p *Plugin) setupWorktree(worktreePath, branchName string) error {
	...
	// 0. Ensure hermes files are in main repo's .gitignore
...
// ensureSidecarGitignore ensures hermes worktree files are in .gitignore.
// This prevents .hermes-agent, .hermes-task, etc. from being
...
func (p *Plugin) ensureSidecarGitignore() error {
	...
	p.ctx.Logger.Info("added hermes entries to .gitignore", "entries", missingEntries)
	return nil
}
