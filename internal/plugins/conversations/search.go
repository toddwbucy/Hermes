...
type SearchFilters struct {
	...
	WorkspaceCWD string    // Filter to sessions matching this working directory
}
...
func (f *SearchFilters) Matches(session adapter.Session) bool {
	...
	// Workspace CWD filter — sessions with empty CWD always pass (backward compat)
	if f.WorkspaceCWD != "" && session.CWD != "" && session.CWD != f.WorkspaceCWD {
		return false
	}
...
func (f *SearchFilters) String() string {
	...
	if f.WorkspaceCWD != "" {
		parts = append(parts, "[workspace:"+filepath.Base(f.WorkspaceCWD)+"]")
	}
