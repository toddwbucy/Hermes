...
import (
	...
	"github.com/toddwbucy/hermes/internal/community"
	"github.com/toddwbucy/hermes/internal/config"
	"github.com/toddwbucy/hermes/internal/keymap"
	...
	"github.com/toddwbucy/hermes/internal/theme"
	"github.com/toddwbucy/hermes/internal/version"
)
...
// Model is the root Bubble Tea model for the hermes application.
type Model struct {
	// Configuration
	cfg *config.Config
...
func (m *Model) initQuitModal() {
	m.quitModal = modal.New("Quit Hermes?",
		modal.WithWidth(50),
...
func (m *Model) runInstallPhase() tea.Cmd {
	...
	return func() tea.Msg {
		...
		if sidecarUpdate != nil {
			switch method {
			case version.InstallMethodHomebrew:
				cmd := exec.Command("brew upgrade hermes")
				...
				if strings.Contains(outLower, "already installed") || strings.Contains(outLower, "already up-to-date") {
					return UpdateErrorMsg{Step: "sidecar", Err: fmt.Errorf("brew upgrade hermes")}
				}
				sidecarUpdated = true
			...
			default: // Go install
				args := []string{
					...
					fmt.Sprintf("github.com/toddwbucy/hermes/cmd/hermes@%s", sidecarUpdate.LatestVersion),
				}
...
func (m *Model) runVerifyPhase(installResult UpdateInstallDoneMsg) tea.Cmd {
	return func() tea.Msg {
		...
		if installResult.SidecarUpdated {
			sidecarPath, err := exec.LookPath("hermes")
			if err != nil {
				return UpdateErrorMsg{Step: "verify", Err: fmt.Errorf("hermes not found in PATH after install")}
			}
			// Verify the binary is executable by running --version
			cmd := exec.Command(sidecarPath, "--version")
			output, err := cmd.Output()
			if err != nil {
				return UpdateErrorMsg{Step: "verify", Err: fmt.Errorf("hermes binary not executable: %v", err)}
			}
...
func (m *Model) copyProjectSetupPrompt() tea.Cmd {
	prompt := `Configure hermes projects for me.

Add my code projects to ~/.config/hermes/config.json using this format:
