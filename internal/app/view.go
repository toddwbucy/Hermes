...
func (m Model) renderHeader() string {
	...
	finalTitleWidth := lipgloss.Width(styles.BarTitle.Render(" Hermes"))
	if m.intro.RepoName != "" {
	...
	if m.intro.Active {
		...
		title = styles.BarTitle.Render(" Hermes") + repoSuffix + worktreeIndicator + " "
	}
...
func (m Model) getTabBounds() []TabBounds {
	...
	titleWidth := lipgloss.Width(styles.BarTitle.Render(" Hermes"))
	if m.intro.RepoName != "" {
...
func (m Model) getRepoNameBounds() (start, end int, ok bool) {
	...
	titlePrefix := styles.BarTitle.Render(" Hermes")
