...
import (
	...
	"github.com/toddwbucy/hermes/internal/markdown"
...
func (m *Model) ensureUpdatePreviewModal() {
	...
	// Hermes update (possibly with td update bundled)
	// Version line
	...
	switch m.updateInstallMethod {
	case version.InstallMethodHomebrew:
		methodHint = styles.Muted.Render("Method: brew upgrade hermes")
		buttons = []modal.ButtonDef{
			modal.Btn(" Update Now ", "update"),
			modal.Btn(" Later ", "cancel"),
		}
	case version.InstallMethodBinary:
		downloadURL := fmt.Sprintf("https://github.com/toddwbucy/hermes/releases/tag/%s",
	...
	m.updatePreviewModal = modal.New("Hermes Update",
		modal.WithWidth(modalW),
		modal.WithVariant(modal.VariantDefault),
		modal.WithPrimaryAction("update"),
	).
...
func (m *Model) renderUpdateProgressModal() string {
	...
	// Title
	title := lipgloss.NewStyle().Bold(true).Foreground(styles.Warning).Render("Updating Hermes")
...
func (m *Model) ensureUpdateCompleteModal() {
	...
	if m.updateAvailable != nil {
		updatesText = fmt.Sprintf("  %s Hermes updated to %s", checkmark, m.updateAvailable.LatestVersion)
	} else {
		updatesText = fmt.Sprintf("  %s Hermes updated", checkmark)
	}
...
func (m *Model) ensureUpdateErrorModal() {
	...
	case version.InstallMethodHomebrew:
		methodName = "Homebrew"
		manualFix = "brew upgrade hermes"
	case version.InstallMethodGo:
		methodName = "go install"
		manualFix = "go install github.com/toddwbucy/hermes/cmd/hermes@latest"
	default:
		methodName = "binary"
		manualFix = "github.com/toddwbucy/hermes/releases"
	}
	...
	reportHint := styles.Muted.Render("Report: github.com/toddwbucy/hermes/issues")
