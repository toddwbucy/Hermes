...
func TestEnsureGitignoreEntries_AddAndIdempotent(t *testing.T) {
	...
	if err := ensureGitignoreEntries(tmp, []string{".todos/", ".hermes/"}); err != nil {
	...
	if strings.Count(content, ".hermes/") != 1 {
		t.Fatalf(".hermes/ count = %d, want 1", strings.Count(content, ".hermes/"))
	}
}
